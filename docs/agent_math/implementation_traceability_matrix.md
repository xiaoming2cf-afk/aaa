...
| --- | --- | --- | --- | --- | --- |
| `tilde_rho_t(k)` retrieval surrogate | Variational | `agent_math/retrieval.py` | Candidate-set-normalized lexical/profile/memory proxy. | Golden-query top-k recall must beat `_knowledge_score`; candidate-set miss audit must be recorded. | `tests/test_agent_math_v2.py` |
...
| `eta_t` delivery readiness | Operational | `agent_math/delivery.py` | Logistic adequacy/governance proxy. | Brier score, expected calibration error, false publish rate, and false block rate must pass release thresholds. | `tests/test_delivery_review.py` |
